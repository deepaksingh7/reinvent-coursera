...
import com.coursera.admin.web.service.LoginInfo;
import com.coursera.admin.web.service.UserInfo;
import com.google.common.base.Strings;

/**
 * LoginController Handle the login form. This is not handled in the index
 * controller to provide flexibility (e.g., the index page could be changed
 * without changing the login form processing).
 */
@Controller
public class LoginController extends AuthenticationBase {
	...
	public String login(@RequestParam("user_name") String user, @RequestParam("password") String password,
		...
		// By default, if there's an error, we go back to the index page (with the login
		// form).
		...
		if (!hasErrors) {
			/*
			 * The user name and password have been entered. Now try the login process.
			 * There are three outcomes: 1. The login succeeds and control passes to the
			 * application page. 2. A password change is required (e.g., for a new user) and
			 * control goes to the change password page. 3. The login fails and we go back
			 * to the index page with the login form.
			 */
			try {
				...
				if (loginInfo != null) {
					if (loginInfo.getNewPasswordRequired()) {
						...
						UserInfo userInfo = new UserInfo(loginInfo.getEmailAddr());
