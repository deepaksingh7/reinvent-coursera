...
import com.coursera.admin.web.service.PasswordRequest;
...
public class ChangePasswordController extends AuthenticationBase {
	...
	/**
	 * Display the change password form. This form is displayed differently for a
	 * forgotten password and for a password change for a logged in user (e.g., a
	 * user with an active session). The changeType variable controls the way the
	 * form is displayed.
	 * 
	 * @return
	 */
	...
	public ModelAndView changePassword(ModelMap modelMap, HttpServletRequest request) {
		if (request.getSession().getAttribute(USER_SESSION_ATTR) != null) {
			// if the user is logged in (they know their password) then the change is just
			// an old to new password
			// change without emailing the password.
			UserInfo info = (UserInfo) request.getSession().getAttribute(USER_SESSION_ATTR);
			if (info != null) {
				modelMap.addAttribute("user_name_val", info.getEmailAddr());
			}
		}
	 ...
	 * @param verifyPassword verify password (this should be exactly the same as
	 *                       newPassword) and allows us to help assure that the
	 *                       password the user typed is the one they intended.
	 * @param redirect
	 * @return
	 */
		...
		if (!hasErrors) {
			try {
				...
				redirect.addFlashAttribute("change_password_error",
						"Error encountered in changing password: " + e.getLocalizedMessage());
			}
		}
		return nextPage;
	}
