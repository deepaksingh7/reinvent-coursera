...
/**
 * ForgotPasswordController This controller handles the page and form that
 * allows a user to request a password reset. Cognito will email a code to the
 * user's email. This code can be used to create a new password. The form for
 * the password reset (using the code) is on the reset_password page.
 */
@Controller
	...
	public String resetPasswordForm(@RequestParam("user_name") String user, RedirectAttributes redirect,
		...
		if (!hasErrors) {
			try {
				...
				redirect.addFlashAttribute("user_name_error",
						"User name error: " + e.getClass().getName() + " " + e.getLocalizedMessage());
			}
		}
		return nextPage;
	}
