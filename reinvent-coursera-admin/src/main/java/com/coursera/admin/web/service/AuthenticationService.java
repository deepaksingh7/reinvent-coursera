...
/**
 * AuthenticationService Provide authentication and other user management
 * services.
 */
public class AuthenticationService implements AuthenticationInterface, CognitoResources {
	...
	/**
	 * <blockquote> After a successful authentication, Amazon Cognito returns user
	 * pool tokens to your app. </blockquote> <blockquote> The ID Token contains
	 * claims about the identity of the authenticated user such as name, email, and
	 * phone_number. </blockquote>
	 * 
	 * @param userName
	 * @param password
	 * @return
	...
	protected SessionInfo sessionLogin(final String userName, final String password)
		...
		if (authResult != null) {
			...
			String idnetityToken = null;
			AuthenticationResultType resultType = authResult.getAuthenticationResult();
			if (resultType != null) {
				accessToken = resultType.getAccessToken();
				idnetityToken = resultType.getIdToken();
				Token.setTokenID(idnetityToken);
			}
			final String challengeResult = authResult.getChallengeName();
			info = new SessionInfo(session, accessToken, challengeResult, idnetityToken);
		}
		return info;
	}
	 ...
	 * initially inactive. An email with a temporary password will be sent to the
	 * user's email address. The user will use this temporary password to login (via
	 * the login page). They will then be redirected to the change password page.
	 * </p>
	 * <p>
	 * This function assumes that the user does not exist (the user name and email
	 * should be unique).
	 * </p>
	 * <p>
	 * The caller should check whether the user already exists and handle that with
	 * the appropriate logic. If the user already exists, this method will throw an
	 * exception.
	 * </p>
	 * <p>
	 * The email_verified attribute (set to true) is very important. Without setting
	 * this attribute, the email address will be treated as unverified. If the email
	 * is not verified, an attempt to reset the password will result in an
	 * unverified email error.
	 * </P>
	 * 
	 * @param userName     The publicly visible user name
	 * @param emailAddress the user's email address. This will be the email address
	 *                     that the temporary password will be sent to.
	 * @param location     The users location (e.g., India, USA, Bangladesh)
	 * @return a UserType object.
	 * 
	 * @throws AWSCognitoIdentityProviderException This is the parent exception for
	 *                                             Cognito errors. The caller should
	 *                                             handle the
	 *                                             UsernameExistsException error. 
	 */
	@Override
		...
		if (emailAddr != null && emailAddr.length() > 0) {
			...
			if (info == null) {
				AdminCreateUserRequest cognitoRequest = new AdminCreateUserRequest().withUserPoolId(poolID)
						.withUsername(userInfo.getEmailAddr())
						.withUserAttributes(new AttributeType().withName(EMAIL).withValue(emailAddr),
								new AttributeType().withName("email_verified").withValue("true"));
				// The AdminCreateUserResult resturned by this function doesn't contain useful
				// information so the
				// result is ignored.
				mIdentityProvider.adminCreateUser(cognitoRequest);
			} else {
				// The caller should have checked that the email address is not already used by
				// a user. If this is not
	...
	/**
	 * Delete an existing user. This code assumes that the operation is logically
	 * permitted (a user is only allowed to delete their own account, after password
	 * verification).
	 */
	@Override
	public void deleteUser(final String userName, final String password) throws AWSCognitoIdentityProviderException {
		...
		if (sessionInfo != null) {
			...
			// the adminDeleteUserRequest returns an AdminDeleteUserResult which doesn't
	 ...
	 * @param userName
	 * @param password
	 * @return a LoginInfo object that includes the access token which is used for
	 *         user operations like change password.
	 * @throws AWSCognitoIdentityProviderException In addition to various other
	 *                                             exceptions, the function will
	...
	public LoginInfo userLogin(final String userName, final String password)
		...
		SessionInfo sessionInfo = sessionLogin(userName, password);
		// The process of sessionLogin should either return a session ID (if the account
		// has not been verified) or a
		// token ID (if the account has been verified).
		if (sessionInfo != null) {
			UserInfo userInfo = getUserInfo(userName);
			loginInfo = new LoginInfo(userInfo);
			// check to see if the password used was a temporary password. If this is the
			// case, the password
			// must be reset.
	...
	public void userLogout(final String userName) throws AWSCognitoIdentityProviderException {
		AdminUserGlobalSignOutRequest signOutRequest = new AdminUserGlobalSignOutRequest().withUsername(userName)
				.withUserPoolId(poolID);
		// The AdminUserGlobalSignOutResult returned by this function does not contain
		// any useful information so the
		// result is ignored.
	 ...
	 * logic, this does not require an emailed code to change the password.
	 * </p>
	 * 
	 * @throws NotAuthorizedException (subclass of
	 *                                AWSCognitoIdentityProviderException) if the
	 *                                password is wrong.
	 */
	@Override
	...
	/**
	 * <p>
	 * Change user's email address. The email address is a special attribute since
	 * it will be used for communication with the user.
	 * </p>
	 * <p>
	 * In this code the user is allowed to change their email addressed without
	 * verification via an emailed code by setting the "email_verified" attribute to
	 * true.
	 * </p>
	 * <p>
	 * Ideally it would be nice to verify the email address via an emailed code,
	 * since this assures that the email address is valid and belongs to the user.
	 * However, I have been unable to figure out how to do this within the Cognito
	 * framework. So we assume that the user entered a correct email address. They
	 * can always change it if an error is discovered.
	 * </p>
	 */
	@Override
	 ...
	 * emailed to the user's email address.
	 * </p>
	 * 
	 * @param passwordRequest a PasswordRequest object that provides the information
	 *                        needed to change the password.
	 * @throws AWSCognitoIdentityProviderException the InvalidPasswordException will
	 *                                             be thrown if the user provides an
	...
	/**
	 * <p>
	 * This function is called to reset the user's password if the password has been
	 * forgotten. The function will result in an email being sent to the user's
	 * email account with a reset code.
	 * </p>
	 * 
	 ...
	 * @return a UserInfo object if the information could be retreived, null
	 *         otherwise.
	 * @throws AWSCognitoIdentityProviderException if the user didn't exist, then
	 *                                             the UserNotFoundException
	 *                                             exception will be thrown. The
	 *                                             other exceptions that are
	 *                                             subclasses of
	 *                                             AWSCognitoIdentityProviderException
	 *                                             are authorization or internal
	 *                                             errors.
	 */
	@Override
	 ...
	 * </p>
	 * 
	 * @return a UserInfo object if the lookup succeeded or null if there was no
	 *         user associated with that email address.
	 */
	@Override
	public UserInfo findUserByEmailAddr(String email) throws AWSCognitoIdentityProviderException {
		...
		if (email != null && email.length() > 0) {
			...
			if (users != null && users.size() > 0) {
				// There should only be a single instance of an email address in the Cognito
				// database
	...
	public boolean hasUser(final String userName) {
		...
		try {
			...
			if (info != null && info.getEmailAddr().length() > 0
					&& info.getEmailAddr().equals(userName)) {
				userExists = true;
			}
